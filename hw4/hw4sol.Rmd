...
plotVarImportance(icu_impute)
plotImputationVariance(icu_impute)
```
...
sapply(icu_train, class)

rf <- randomForest(factor(thirty_day_mort) ~ 
                  gender + anchor_age + marital_status + ethnicity + hematocrit
                  , data=icu_train)
```
...
(1) Logistic regression
```{r, warning = FALSE}
probabs <- predict(logistic, icu_test, type='response') 
preds <- ifelse(probabs > 0.5, 1, 0)
library(caret)
confusionMatrix(factor(preds), factor(icu_test$thirty_day_mort))
```
