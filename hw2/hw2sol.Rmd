...
```{r}
mimic_icu_cohort %>% 
  ggplot(mapping = aes(x = ethnicity, fill = thirty_day_mort)) +
  geom_bar(position="dodge") + 
  scale_x_discrete(labels = abbreviate) + 
  labs(fill = "30 day mortality", x = "Ethnicity") +
  theme_minimal()
mimic_icu_cohort %>% 
...
mimic_icu_cohort %>% 
  ...
  theme_minimal()
mimic_icu_cohort %>% 
  ggplot(mapping = aes(y = age_admission, x = thirty_day_mort, fill = "red")) +
  geom_boxplot() + 
